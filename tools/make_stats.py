...
def recover_observer_net(file):
  ...
  def recover_stats(position, limit, data, results, pps, name):
    ...
    for i in range(new_position,limit):
      ...
      interval_length = float(interval_length[1])-float(interval_length[0])
  ...
  with open(file) as f:
    ...
    while cnt < total:
      ...
      if "pps" in lines[cnt]:
        pps_value = lines[cnt][lines[cnt].find(" "):lines[cnt].find("pps")]
        read_pps = True
        packets[pps_value] = dict()
      # Look for stats based on tag
      if read_pps:
        cnt = recover_stats(cnt,total,lines,packets,pps_value, "sent")
        cnt = recover_stats(cnt,total,lines,packets,pps_value, "recv")
        read_pps = False
      # Move forward
      cnt = cnt + 1
...
def get_mem_usage(dirs, outDir):
  ...
  def read_and_split(parent, rep):
    ...
    res = dict()
    for name in files:
      server = name.replace("mem-usage-","").replace(".log","").replace(rep,"")
      res[server] = dict()
  ...
  def write_csv(name,stats):
    ...
    with open(name,"w") as f:
      ...
      for i in range(len(servers)):
        f.write("{},{},{}\n".format(servers[i],stats[servers[i]]["ram"],stats[servers[i]]["swap"]))
...
def get_server_times(dirs, outDir):
  ...
  def write_csv(name,s,avg,std,med,var):
    with open(name,"w") as f:
      f.write("{},{},{},{},{}\n".format("server","avg (ms)","std (ms)","median (ms)", "var (ms)"))
      for i in range(len(s)):
        f.write("{},{},{},{},{}\n".format(s[i],avg[i],std[i],med[i],var[i]))
  
  ra_s, ra_avg, ra_std, ra_median, ra_var = read_and_split(rand)
  re_s, re_avg, re_std, re_median, re_var = read_and_split(real)

  write_csv(outDir+"/random-times.csv",ra_s, ra_avg, ra_std, ra_median, ra_var)
  write_csv(outDir+"/real-times.csv",re_s, re_avg, re_std, re_median, re_var)
  ...
  def unpack_and_save(stats,parent_dir,file_name_suffix):
    ...
    for server in stats.keys():
      ...
      with open(parent_dir+"/mem-evolution-"+file_name_suffix+".csv",op_mode) as f:
        ...
        for machine in mem.keys():
          ...
          for pps in rates:
            ...
            f.write("{},{},{},{}\n".format(server,machine,pps,values))
      # Save cpu evolution
      ...
      with open(parent_dir+"/throughput-evolution-"+file_name_suffix+".csv",op_mode) as f:
        if run_one:
          f.write("target server,operation,pps,timespan,total packets,machine 1,machine 2,machine 3\n")
        ...
        for op in operations:
          for pps in rates:
            ...
            for i in range(entries):
              timespan = timespan + wireshark[pps][op]["interval"][i]
            ...
            f.write("{},{},{},{},{},{},{},{}\n".format(server,op,pps,timespan,packets,m1,m2,m3))
      run_one = False
              
  unpack_and_save(make_stats_per_server(rand,name),outDir,"random-"+name)
  unpack_and_save(make_stats_per_server(real,name),outDir,"real-"+name)

def get_backend_resistance(dirs, outDir):
  get_response_stats(dirs,outDir,"segfault")
...
def main(resultsDir, outDir):
  ...
  get_mem_usage(mem_dir, outDir + "/mem-usage")
  get_server_times(times_dir, outDir + "/time-response")
  get_response_stats(response_dir, outDir + "/response-stats")
  get_backend_resistance(segfault_dir, outDir + "/segfault-stats")
  
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description="Make stats from results after a tests is complete. The results will be some charts and csv files for ease of use.")
  parser.add_argument("directory", type=str, help="Directory containing the results")
  parser.add_argument("outputdir", type=str, help="Where to put the computed stats", default="../stats")
