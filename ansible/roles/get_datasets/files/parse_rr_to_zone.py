        ...
        for domain in domain_dict.keys():
            ...
            with open(file_name, mode) as file:
                ...
                for subdomain in sub_domains:
                    if subdomain not in rrs:
                        ...
                        for record in records:
                            ...
                            if record == "TXT":
                                values.append(
                                    current_sub_dom+domain + "." + tld + ".  IN  "+record+"  "+records[record]+"\n")
                            elif record == "SOA":
                                ...
                                for r_value in record_values:
                                    values.append(
                                        current_sub_dom+domain + "." + tld + ".  IN  "+record+"  "+r_value+"\n")
                    else:
                        # Abort
                        print("wait what")
                        print(subdomain, domain, tld)
                        exit(0)
                        values.append(domain + "." + tld + ".  IN  " +
                                      subdomain+"  "+sub_domains[subdomain]+"\n")
                #
                #  Save to file
                #
                ...
                if soa_recovered == "":
                    ...
                    c_soas = c_soas + 1
                file.write(domain + "." + tld +
                           ".  IN  SOA  "+soa_recovered+"\n")
                c_rr = c_rr + 1
                # Verify that there's at least one NS RR
                add_ns = True
                for rr in values:
                    if "NS" in rr:
                        add_ns = False
                ...
                if add_ns or add_soa_rr:
                    ...
                    file.write("ns1."+domain + "." + tld +
                               ".  IN  A  192.0.0.1\n")
                    file.write(domain + "." + tld +
                               ".  IN  MX  10 mail."+domain+"."+tld+".\n")
                    file.write("mail."+domain + "." +
                               tld + ".  IN  A  192.0.0.2\n")
                    c_rr = c_rr + 4
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Recover Zones from a list of RR to multiple files")
    parser.add_argument("f", type=str, help="File to read RR from")
    parser.add_argument('-o', "--output_file",
                        help="Optional single zone file name", default=False)
    ...
    zones, soa_used, written_rr = exportDomainsToFile(
        rr_tree, args.output_file)
    logger.info("Created %d zones, recovered %d soa records, wrote %d rr",
                zones, soa_used, written_rr)
